...
- 등록된 툴데이터(무게/무게중심/이너셔)를 읽거나 해당 툴데이터를 변경합니다.
- 툴데이터가 등록되지 않았거나 멤버가 유효하지 않은 경우, 에러가 발생하면서 job 실행이 중단됩니다.
...
- E14550 : 툴데이터의 멤버가 유효하지 않은 경우 발생합니다. 설정된 툴데이터의 맴버가 mass, cx, cy, cz, ixx, iyy, izz인지 확인하십시오.
- E14286 : 대입문 우변으로 시프트변수가 아닌 경우이거나 툴변수의 맴버가 유효하지 않은 경우에 발생합니다. 대입문 우변을 다시 지정하십시오.
